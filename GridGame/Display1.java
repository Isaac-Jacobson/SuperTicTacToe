...
public class Display1 {
	...
	public Display1(PApplet p, int x, int y, int w, int h) {
		this.x = x;
		this.y = y;
		this.w = w;
		this.h = h;
	...
	public void highlightLocation(Location1 l, TicTacToe g) {
		if (g.isInGrid(l)) {
	...
	private float yCoordOf(Location1 l) {
		return y + l.getRow() * dy;
	}

	// return the x pixel value of the upper-left corner of location l
	private float xCoordOf(Location1 l) {
		return x + l.getCol() * dx;
	}

	// Return location at coordinates x, y on the screen
	public Location1 gridLocationAt(float mousex, float mousey) {
		Location1 l = new Location1((int) Math.floor((mousey - y) / dy),
	...
	public void initializeWithGame(TicTacToe game) {
